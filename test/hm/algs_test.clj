  ...
  (testing "inference normal types"
    (let [fun-id    (EAbs "x" (EVar "x"))
      ...
      (is= (s-of-t (infer {} expr11))
           "(c → d) → ((d → e) → (c → e))")
      (is= (s-of-t (infer common-env expr12)) "int")
      (is= (s-of-t (infer common-env expr13)) "int → bool")
      (is= (s-of-t (infer common-env expr14)) "types do not unify: bool vs. int in true + false")
      (is= (s-of-t (infer common-env expr15)) "(b → (h → i)) → (b → ((g → h) → (g → i)))")
      (is= (s-of-t (infer common-env expr16)) "bool")))
  (testing "inference compound types"
    (let [expr0  (EPair (ELit (LInt 3))
          ...
          expr10 (EPair (ELit (LString "term"))
                        (ELit (LInt 3)))]
      (is= (s-of-t (infer common-env expr0)) "(int * bool)")
      (is= (s-of-t (infer common-env expr1)) "unbound variable: f in (f 3, f 3)")
      (is= (s-of-t (infer common-env expr2)) "(int → c) → (c * c)")
      (is= (s-of-t (infer common-env expr3)) "types do not unify: int vs. bool in λf → (f 3, f true)")
      (is= (s-of-t (infer common-env expr4)) "(int * bool)")
      (is= (s-of-t (infer common-env expr5)) "f → (f * f)")
      (is= (s-of-t (infer common-env expr6)) "int")
      (is= (s-of-t (infer common-env expr7)) "[e] → int")
      (is= (s-of-t (infer common-env expr8)) "(int * bool)")
      (is= (s-of-t (infer common-env expr9)) "types do not unify: bool vs. int in λid → (id true, id 3)")
                                        ...
                                        (EIf (EIsZero (EVar "n"))
                                             ...
                                             (ETimes (EVar "n")
                                                     (EApp (EVar "factorial")
                                                           (EPred (EVar "n"))))))))
                      (EApp (EVar "factorial") (ELit (LInt 5))))]
      (is= (s-of-t (infer common-env expr0)) "int → int")
      (is= (s-of-t (infer common-env expr1)) "int")
      (is= (s-of-t (infer common-env expr2)) "int → int")
      (is= (s-of-t (infer common-env expr3)) "int"))))
