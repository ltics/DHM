...
(deftest algw-test
  ...
  (testing "inference normal types"
    (let [fun-id    (EAbs 0 (EVar 0))
      ...
      (is= (s-of-t (infer common-env expr14)) "types do not unify: int vs. bool in add true")
      (is= (s-of-t (infer common-env expr15)) "(a → (e → f)) → (a → ((d → e) → (d → f)))")
  ...
  (testing "inference compound types"
    (let [expr0  (EApp (EApp (EVar "pair")
      ...
      (is= (s-of-t (infer common-env expr1)) "unbound variable: f")
      (is= (s-of-t (infer common-env expr2)) "(int → f) → (f * f)")
      (is= (s-of-t (infer common-env expr3)) "types do not unify: int vs. bool in f true")
      (is= (s-of-t (infer common-env expr4)) "(int * bool)")
      (is= (s-of-t (infer common-env expr5)) "i → (i * i)")
      (is= (s-of-t (infer common-env expr6)) "int")
      (is= (s-of-t (infer common-env expr7)) "[d] → int")
      (is= (s-of-t (infer common-env expr8)) "(int * bool)")
      (is= (s-of-t (infer common-env expr9)) "types do not unify: bool vs. int in id 3")
      (is= (s-of-t (infer common-env expr10)) "(string * int)")))
