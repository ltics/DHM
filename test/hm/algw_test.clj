...
(deftest algw-test
  ...
  (testing "inference"
    (let [fun-id    (EAbs 0 (EVar 0))
      ...
      (is= (s-of-m (infer {} expr5)) "occurs check fails: E vs. (E -> F)")
      (is= (s-of-m (infer {} expr6)) "((bool -> H) -> H)")
      (is= (s-of-m (infer {} expr7)) "types do not unify: int vs. (int -> I)")
      (is= (s-of-m (infer {} expr8)) "((int -> Q) -> Q)")
      (is= (s-of-m (infer {} expr9)) "occurs check fails: U vs. (U -> V)"))))
